<template>
  ...
  <div>
    ...
    <span v-if="yearList[index] && !hoverSelect">{{ yearList[index].min }}<br>•<br>{{ yearList[index].max }}</span>
    <span v-else-if="hoverSelect">{{ hoverSelect.min }}<br>•<br>{{ hoverSelect.max }}</span>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      hover (i) {
        this.hoverSelect = {...this.yearList[i]}
      },
